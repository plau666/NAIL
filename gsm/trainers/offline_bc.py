"""Offline behavior cloning baseline for GSM8K LoRA experiments.

This trainer implements LogLossBC: supervised fine-tuning of the student on
pre-generated teacher rollouts. Unlike the KL trainers, it is fully off-policy
and does not query the expert during training. The student is wrapped with PEFT
LoRA adapters, and Hugging Face Trainer handles batching, optimization, and
checkpoint state.

Training examples are JSONL records with prompt and completion fields. The
default launcher expects teacher rollouts generated by `data/teacher_rollout.py` at
`data/teacher_rollouts/train.jsonl`.

Usage:
    python trainers/offline_bc.py \
    ...
    python trainers/offline_bc.py ... --resume_from_checkpoint auto

    # Resume from specific checkpoint:
    python trainers/offline_bc.py ... --resume_from_checkpoint output/obc_lora_r128/checkpoint-200
"""
...
def main():
    ...
    # resume_from_checkpoint restores model, optimizer, scheduler, trainer
    # state, and RNG from the checkpoint directory.
