#!/bin/bash
# Reverse-KL on-policy distillation (NAIL-R / OPD-R) with LoRA: importance-weighted score-function surrogate.
# Logs GSM8K test eval_loss every save_steps.
#
# Usage (from inside gsm/):
#   bash run_reverse_lora.sh
#
# Override via env vars:
#   GPU=3 STUDENT_TEMP=1.0 EXPERT_TEMP=1.0 bash run_reverse_lora.sh
...
echo "=== Reverse-KL + LoRA (rank=${LORA_RANK}) ==="
...
PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=$GPU \
    ...
    nohup python "$SCRIPT_DIR/reverse_lora_vllm.py" \
