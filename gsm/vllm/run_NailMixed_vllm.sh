...
#   EXPERT_TEMP  = 4.0  (noisy expert; same default as run_NailR.sh /
#                        run_NailF.sh — set EXPERT_TEMP=1.0 for clean expert)
#
# Usage (from inside gsm/):
#   bash run_NailMixed.sh                              # beta=0.5
#   BETA=0.3 bash run_NailMixed.sh                     # 70% F / 30% R
#   BETA=0.7 EXPERT_TEMP=1.0 bash run_NailMixed.sh     # clean expert, R-heavy
#
...
PYTHONUNBUFFERED=1 CUDA_VISIBLE_DEVICES=$GPU \
    ...
    nohup python "$SCRIPT_DIR/mixed_lora_vllm.py" \
