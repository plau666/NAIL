"""Offline Behavioral Cloning with LoRA: SFT a student model on pre-generated rollouts
using low-rank adapters, with full training-state checkpointing for resume.

Differences from offline_bc.py:
  ...
  - Uses HF Trainer which automatically saves optimizer/scheduler/RNG/step state
    inside each checkpoint directory, enabling `--resume_from_checkpoint`.

Usage:
    python offline_bc_lora.py \
        --student_model google/gemma-3-270m-it \
        --train_data real_math/data/rollouts/.../train.jsonl \
        --output_dir real_math/output/obc_lora_r16 \
        --name obc_lora_r16 \
        --lora_rank 16
    ...
    python offline_bc_lora.py ... --resume_from_checkpoint real_math/output/obc_lora_r16/checkpoint-200
"""
...
def main():
    ...
    parser.add_argument("--wandb_project", type=str, default="real_math")
